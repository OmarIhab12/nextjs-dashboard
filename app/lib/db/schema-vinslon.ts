// app/lib/db/schema-vinslon.ts
// Canonical schema for fresh databases.
// For existing databases, run migrations in order:
//   1. npx tsx app/lib/db/migrate-wallet-accounts.ts
//   2. npx tsx app/lib/db/migrate-rmb.ts
// Run fresh: npx tsx app/lib/db/schema-vinslon.ts
...
async function schema() {
  ...
  await sql`DO $$ BEGIN CREATE TYPE wallet_currency        AS ENUM ('EGP', 'USD', 'RMB');                                EXCEPTION WHEN duplicate_object THEN NULL; END $$`;
  ...
  await sql`
    CREATE TABLE IF NOT EXISTS company_wallet (
      ...
      rmb_balance NUMERIC(14, 2) NOT NULL DEFAULT 0.00,
      updated_at  TIMESTAMPTZ    NOT NULL DEFAULT NOW()
    )
  `;
  await sql`
    INSERT INTO company_wallet (egp_balance, usd_balance, rmb_balance)
    SELECT 0.00, 0.00, 0.00 WHERE NOT EXISTS (SELECT 1 FROM company_wallet)
  `;

  // ── Wallet Accounts (per-currency, per-method balances) ───────────────────────
  await sql`
  ...
  `;
  // Seed all 12 accounts (3 currencies × 4 payment methods)
  await sql`
    INSERT INTO wallet_accounts (currency, method, balance)
    VALUES
      ('EGP', 'bank_transfer', 0), ('EGP', 'cash', 0), ('EGP', 'check', 0), ('EGP', 'vodafone_cash', 0),
      ('USD', 'bank_transfer', 0), ('USD', 'cash', 0), ('USD', 'check', 0), ('USD', 'vodafone_cash', 0),
      ('RMB', 'bank_transfer', 0), ('RMB', 'cash', 0), ('RMB', 'check', 0), ('RMB', 'vodafone_cash', 0)
    ON CONFLICT (currency, method) DO NOTHING
  `;

  // ── Wallet Transfers (move funds between accounts, same currency) ──────────────
  await sql`
  ...
  // On a reversal → points to the original. On a correction → points to the reversal.
  await sql`
  ...
  // ── Currency Conversions (multi-currency) ──────────────────────────────────────
  // from_amount: amount debited (from_currency)
  // to_amount:   amount credited (to_currency)
  // direction encodes both currencies; trigger uses CASE to route balances
  await sql`
  ...
  // ── Orders (us → supplier, paid in RMB) ───────────────────────────────────────
  await sql`
    CREATE TABLE IF NOT EXISTS orders (
      id          UUID           PRIMARY KEY DEFAULT uuid_generate_v4(),
      supplier_id UUID           REFERENCES suppliers(id) ON DELETE SET NULL,
      total_rmb   NUMERIC(14, 2) NOT NULL CHECK (total_rmb > 0),
  ...
  // ── Order Payments (RMB sent to supplier) ──────────────────────────────────────
  await sql`
  ...
  await sql`CREATE INDEX IF NOT EXISTS idx_wallet_transfers_at       ON wallet_transfers(transferred_at)`;
  ...
  for (const table of [
    ...
    'suppliers', 'orders', 'order_instalments', 'wallet_accounts',
  ]) {
    await sql.unsafe(`
  ...
  await sql`
    ...
    BEGIN
      INSERT INTO order_instalments (
        ...
        NEW.id, 1, NEW.total_rmb, 0, NEW.total_rmb, 'pending'
      );
      RETURN NEW;
    END;
    $$
  ...
  // ── Stock: update on order status → stored ────────────────────────────────────
  await sql`
  ...
  // ── Stock: order_items changes (only when order is stored) ────────────────────
  await sql`
    CREATE OR REPLACE FUNCTION update_stock_on_order_item_change()
    RETURNS TRIGGER LANGUAGE plpgsql AS $$
    DECLARE v_status TEXT;
    BEGIN
      SELECT status::TEXT INTO v_status FROM orders WHERE id = COALESCE(NEW.order_id, OLD.order_id);
      IF v_status <> 'stored' THEN RETURN COALESCE(NEW, OLD); END IF;
  ...
  await sql`
    CREATE OR REPLACE TRIGGER trg_stock_on_order_item_insert
      AFTER INSERT ON order_items FOR EACH ROW EXECUTE FUNCTION update_stock_on_order_item_change()
  `;
  await sql`
    CREATE OR REPLACE TRIGGER trg_stock_on_order_item_update
      AFTER UPDATE ON order_items FOR EACH ROW EXECUTE FUNCTION update_stock_on_order_item_change()
  `;
  await sql`
    CREATE OR REPLACE TRIGGER trg_stock_on_order_item_delete
      AFTER DELETE ON order_items FOR EACH ROW EXECUTE FUNCTION update_stock_on_order_item_change()
  `;

  // ── Wallet Transfers: update account balances ──────────────────────────────────
  await sql`
  ...
  // ── Wallet: currency conversions (multi-currency, always via bank_transfer) ─────
  // from_amount/to_amount + direction → determine which balances to debit/credit
  await sql`
    ...
    BEGIN
      ...
      INSERT INTO wallet_transactions
        (currency, amount, direction, reason, reference_id, created_at, account_id)
      SELECT from_currency::wallet_currency, NEW.from_amount, 'out', 'conversion', NEW.id, NEW.converted_at, wa.id
      FROM wallet_accounts wa
      ...
      INSERT INTO wallet_transactions
        (currency, amount, direction, reason, reference_id, created_at, account_id)
      SELECT to_currency::wallet_currency, NEW.to_amount, 'in', 'conversion', NEW.id, NEW.converted_at, wa.id
      FROM wallet_accounts wa
  ...
  // ── Wallet: order payments (RMB out on INSERT, RMB in on DELETE) ───────────────
  await sql`
